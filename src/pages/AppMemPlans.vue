<template>
            ...
            <div class="card-body">
              ...
              <div class="row">
                ...
                <div class="col-6" style="text-align: right;">
                  <button @click="new_item()" class="btn btn-primary">Add Plan</button>
                </div>
              </div>
              <div class="row">
                <div class="col-12">
                    ...
                    <table id="order-listing" class="table">
                      ...
                      <tbody>
                        <tr v-for="user in l1" :key="user">
                          ...
                          <td>{{ user.plan_duration }} Days</td>
                          <td>{{ user.plan_price }}</td>
                         
                          <td>
                            <a href="javascript:void(0)" @click="edit_item(user.plan_id)">
                              <label class="badge badge-info">Edit</label>
                            </a>
                            &nbsp;&nbsp;&nbsp;
                            <a href="javascript:void(0)" @click="del_item1(user.plan_id)">
  ...
  <script>
    ...
    import { getPlanList,deletePlanById } from '@/apis/memplanapis';
    ...
    export default {
      ...
      methods: {
        ...
        async getListNow($page_no) {
                ...
                const response = await getPlanList(this.page_no,this.count,this.ldata.user_id);
                if (response.status) {
        ...
        edit_item($id){
          ...
          window.location.href = "/new_mem_plan";
        },
        new_item(){
        ...
        async del_item() {
                const response = await deletePlanById(this.item_id);
                if (response.status) {
